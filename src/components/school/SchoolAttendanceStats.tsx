    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <select
          ...
          style={{ height: 32, fontSize: 12, borderRadius: 7, border: `1px solid ${BORDER}`, padding: "0 8px", color: TEXT, background: "var(--color-surface)" }}
        >
          <option value="week">최근 7일</option>
          <option value="month">이번 달</option>
        </select>
      </div>
      <div style={{ background: "var(--color-surface)", borderRadius: 7, border: `1px solid ${BORDER}`, padding: 16 }}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {stats.map((s) => (
            <div key={s.department_id} style={{ padding: 10, borderRadius: 7, border: `1px solid ${BORDER}`, background: "var(--color-surface)" }}>
