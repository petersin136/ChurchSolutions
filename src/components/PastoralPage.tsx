...
function normMemberMgmtSearchText(s: string): string {
  return (s ?? "").toLowerCase().replace(/\s+/g, "").replace(/[,\uFF0C./]/g, "");
}

function memberMokjangLabelForSearch(m: Member): string {
  return (m.mokjang ?? m.group) || "";
}

function filterMembersMgmtSearch(members: Member[], query: string): Member[] {
  const q = query.trim().replace(/[,\uFF0C]+$/g, "").trim();
  if (!q) return [];
  const qn = normMemberMgmtSearchText(q);
  const digits = q.replace(/\D/g, "");
  if (digits.length === 0 && q.length < 2) {
    return members.filter((m) => normMemberMgmtSearchText(m.name || "").includes(qn));
  }
  return members.filter((m) => {
    const nameN = normMemberMgmtSearchText(m.name || "");
    const phoneN = (m.phone || "").replace(/\D/g, "");
    const deptN = normMemberMgmtSearchText(m.dept || "");
    const mokjangN = normMemberMgmtSearchText(memberMokjangLabelForSearch(m));
    const roleN = normMemberMgmtSearchText(m.role || "");
    const addrN = normMemberMgmtSearchText(m.address || "");
    const memoN = normMemberMgmtSearchText(m.memo || "");
    const prayerN = normMemberMgmtSearchText(m.prayer || "");
...
function MembersSub({ db, setDb, persist, toast, currentWeek, openMemberModal, openNoteModal, openQuickNote, openActivityModal, churchId }: {
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [db.members, deptF, roleF, mokjangF, statusF, riskDormantOnly]);

  const searchMemberMatches = useMemo(() => {
    if (!searchQuery.trim()) return [];
    return filterMembersMgmtSearch(filtered, searchQuery)
      .slice(0, 8)
      .map((m) => ({
        id: m.id,
        name: m.name,
        subtitle: [m.role, m.dept, memberMokjangLabelForSearch(m)].filter(Boolean).join(" · "),
      }));
  }, [filtered, searchQuery]);

  const handleSelectSearchMember = useCallback(
    (memberId: string) => {
      const idx = filtered.findIndex((m) => m.id === memberId);
      if (idx < 0) return;
      setPageList(Math.floor(idx / PAGE_SIZE_MEM) + 1);
      setSearchQuery("");
    },
    [filtered, PAGE_SIZE_MEM],
  );
  ...
  return (
    <MembersManagementPanel
      ...
      db={db}
      filtered={filtered}
      searchMemberMatches={searchMemberMatches}
      onSelectSearchMember={handleSelectSearchMember}
