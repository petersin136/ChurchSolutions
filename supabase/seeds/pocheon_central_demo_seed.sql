...
FROM (
  ...
  UNION ALL SELECT 'workflows',               COALESCE((SELECT COUNT(*)::INT FROM workflows               WHERE church_id = (SELECT id FROM ch)), 0)
  UNION ALL SELECT 'workflow_cards',          COALESCE((SELECT COUNT(*)::INT FROM workflow_cards          WHERE church_id = (SELECT id FROM ch)), 0)
) t
ORDER BY table_name;
...
-- 사역흐름 시드 (5개 기본 + 카드 샘플)
-- ---------------------------------------------------------------------
-- supabase/seeds/pocheon_central_workflow_seed.sql 과 동일 내용.
-- 본 시드를 처음부터 실행할 때 워크플로우까지 자동 채워지도록 포함.
-- 워크플로우/스텝은 (church_id, template_key) / (workflow_id, sort_order)
-- 유니크 제약으로 멱등, 카드는 source_ref 로 중복 가드.
-- =====================================================================
DO $$
DECLARE
  v_church_id UUID;
...
BEGIN
  ...
  IF v_church_id IS NULL THEN
    RAISE NOTICE '⚠️ 워크플로우 시드 건너뜀: 포천중앙침례교회를 찾지 못함';
    RETURN;
  END IF;
  ...
  IF v_wid IS NULL THEN
    SELECT id INTO v_wid FROM public.workflows WHERE church_id = v_church_id AND template_key = 'new_family' LIMIT 1;
  END IF;
  ...
  i := 0;
  FOR v_m IN SELECT id, name, phone FROM members WHERE church_id = v_church_id ORDER BY created_at DESC NULLS LAST LIMIT 6 LOOP
    i := i + 1;
    ...
    SELECT v_church_id, v_wid,
      ...
      'normal',
      now() - (i || ' days')::interval,
      'auto_new_family', 'seed_pc_nf_' || i
    WHERE NOT EXISTS (
      SELECT 1 FROM workflow_cards WHERE church_id = v_church_id AND source_ref = 'seed_pc_nf_' || i
    );
  END LOOP;
  ...
  RETURNING id INTO v_wid;
  IF v_wid IS NULL THEN
    SELECT id INTO v_wid FROM public.workflows WHERE church_id = v_church_id AND template_key = 'absentee_recovery' LIMIT 1;
  END IF;
  INSERT INTO public.workflow_steps (church_id, workflow_id, name, description, sort_order, expected_days, is_terminal) VALUES
  ...
  SELECT id INTO v_step3 FROM workflow_steps WHERE workflow_id = v_wid AND sort_order = 3;
  i := 0;
  FOR v_m IN SELECT id, name, phone FROM members WHERE church_id = v_church_id AND dept = '장년부' ORDER BY name LIMIT 4 LOOP
    i := i + 1;
    INSERT INTO public.workflow_cards (
      church_id, workflow_id, current_step_id, member_id, member_name, member_phone,
      stage, priority, moved_to_step_at, source, source_ref
    ...
    SELECT v_church_id, v_wid,
      ...
      v_m.id, v_m.name, v_m.phone,
      'open', CASE WHEN i = 1 THEN 'high' ELSE 'normal' END,
      now() - (i * 2 || ' days')::interval,
      'auto_absentee', 'seed_pc_ar_' || i
    WHERE NOT EXISTS (
      SELECT 1 FROM workflow_cards WHERE church_id = v_church_id AND source_ref = 'seed_pc_ar_' || i
    );
  END LOOP;
  ...
  RETURNING id INTO v_wid;
  IF v_wid IS NULL THEN
    SELECT id INTO v_wid FROM public.workflows WHERE church_id = v_church_id AND template_key = 'baptism' LIMIT 1;
  END IF;
  INSERT INTO public.workflow_steps (church_id, workflow_id, name, description, sort_order, expected_days, is_terminal) VALUES
  ...
  RETURNING id INTO v_wid;
  IF v_wid IS NULL THEN
    SELECT id INTO v_wid FROM public.workflows WHERE church_id = v_church_id AND template_key = 'ordination' LIMIT 1;
  END IF;
  INSERT INTO public.workflow_steps (church_id, workflow_id, name, description, sort_order, expected_days, is_terminal) VALUES
  ...
  RETURNING id INTO v_wid;
  IF v_wid IS NULL THEN
    SELECT id INTO v_wid FROM public.workflows WHERE church_id = v_church_id AND template_key = 'reactivation' LIMIT 1;
  END IF;
  INSERT INTO public.workflow_steps (church_id, workflow_id, name, description, sort_order, expected_days, is_terminal) VALUES
